...
class LocalAgentMixin:  # pylint: disable=too-few-public-methods
    ...
    MAX_AGENT_RECONNECT_ATTEMPTS = 5
    MIN_AGENT_RECONNECT_DELAY = 2
    ...
    async def _start_local_agent_listener(self):
        ...
        while True:
            ...
            if not await self.__attempt_to_listen(context):
                ...
                if agent_connection_drops >= self.MAX_AGENT_RECONNECT_ATTEMPTS:
                    logger.warning(
                        "Max agent reconnect attempts (%s) reached. Giving up.",
                        self.MAX_AGENT_RECONNECT_ATTEMPTS,
                    )
                    return
                # nosemgrep: gitlab.bandit.B311
                sleep_seconds = random.uniform(self.MIN_AGENT_RECONNECT_DELAY, 10)  # nosec B311
                logger.warning(
    ...
    def _async_start_local_agent_listener(self):
        ...
        future = asyncio.run_coroutine_threadsafe(
            self._start_local_agent_listener(), self._asyncio_loop
        )
    ...
    def _async_stop_local_agent_listener(self):
        ...
        """
        future = asyncio.run_coroutine_threadsafe(
            self._agent_listener.stop(), self._asyncio_loop
        )
        future.add_done_callback(self._handle_future_result)
    ...
    def __handle_hard_jailed_state(self, status: Status):
        context = EventContext(
            connection=self, connection_details=status.connection_details
        )
        ...
        elif self.__has_reached_max_amount_of_concurrent_vpn_connections(
            status.reason.code
        ):
    ...
    async def __attempt_to_request_connection_features(
    ...
    def __get_agent_features(self, features: Features) -> AgentFeatures:
        randomized_nat = (
            not features.moderate_nat if features.moderate_nat is not None else None
        )
